...
def test_str_has_expected_format():
    email = Email(
        subject="Hello",
        body="World",
        sender=EmailAddress("a@a.com"),
...
def test_send_email_single_ready():
    email = Email("Hello", "Msg", EmailAddress("a@a.com"), EmailAddress("b@b.com"))
    email.prepare()  # Статус становится READY
...
def test_status_transitions():
    ...
    service = EmailService()
    sent = service.send_email(email)[0]
    assert sent.status == Status.SENT